    ...
    <div className="service-page">
        ...
        <div className="container">
          <div className="service-hero-inner">
            <div className="service-hero-text">
              ...
              <p>We design websites, apps, and digital products that are beautiful, intuitive, and built around your users. Great design isn't just how it looks — it's how it works.</p>
              <Link href="/contact" className="service-hero-cta">Start a Design Project <i className="bi bi-arrow-right"></i></Link>
            </div>
            <div className="service-hero-image">
              <NextImage src="/assets/img/service-hero.jpg" alt="UI/UX Design Services" width={420} height={320}
                style={{ width: '100%', height: '320px', objectFit: 'cover', display: 'block' }} priority />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="service-process">
        <div className="container">
          <div className="text-center mb-5">
            ...
            <p className="section-sub mx-auto">A structured, collaborative process that delivers great design every time.</p>
          </div>
            ...
            {process.map((step, i) => {
              ...
              return (
                <div key={i} className={`process-timeline__row ${isLeft ? 'process-timeline__row--left' : 'process-timeline__row--right'}`}>
                  <motion.div className="process-timeline__card"
                    initial={{ opacity: 0, x: isLeft ? -60 : 60 }} whileInView={{ opacity: 1, x: 0 }}
                    viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.55, delay: i * 0.08 }}>
                    <h4>{step.title}</h4><p>{step.desc}</p>
                  </motion.div>
                  <motion.div className="process-timeline__bubble"
                    initial={{ opacity: 0, scale: 0.3 }} whileInView={{ opacity: 1, scale: 1 }}
                    viewport={{ once: true, amount: 0.3 }} transition={{ type: 'spring', stiffness: 300, damping: 18, delay: i * 0.08 + 0.2 }}>
                    {i + 1}
                  </motion.div>
              ...
              {faqs.map((faq, i) => (
                <div key={i} className="faq-item">
                  <button className={`faq-question ${openFaq === i ? 'open' : ''}`} onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                    {faq.q}<i className="bi bi-chevron-down"></i>
                  </button>
      ...
      <section className="service-cta">
        <div className="container">
          ...
          <Link href="/contact" className="service-cta-btn">Get in Touch <i className="bi bi-arrow-right"></i></Link>
        </div>
      </section>
    </div>
  );
